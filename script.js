...
const inputLogin = document.querySelector('#welcome_input_email'); 
const inputPassword = document.querySelector('#welcome_input_pass'); 
 
const regBtn = document.querySelector('#reg-btn'); 
 
function MainConstructor() { 
    this.doesUserExisted = false; 
    ...
    this.saveReg = function () { 
        const save = {email: inputEmail.value, password: inputPass.value, repeatPassword: inputPassRepeat.value}; 
        if(!this.doesUserExisted){ 
            regInfo.push(save); 
        } 
    }; 
    this.mailValid = function () { 
        this.doesUserExisted = regInfo.find(user => { 
            return user.email === inputLogin.value 
          })
        this.doesUserExisted2 = regInfo.find(user => { 
            return user.password === inputPassword.value 
          })   
          if(!this.doesUserExisted && !this.doesUserExisted2) { 
            alert("Вы вошли в систему") 
            this.doesUserExisted = false;
            this.doesUserExisted2 = false; 
          }else {alert("Юзер такой есть")
        } 
    }; 
};
